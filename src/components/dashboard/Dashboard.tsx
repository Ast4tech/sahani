...
const handleToggleFavorite = async (recipeId: Id<"recipes">) => {
  ...
  const pendingShoppingItems =
    currentList?.items.filter((i) => !i.checked).slice(0, 4) || [];
...
const nextRecipe = useQuery(
  api.recipes.get,
  nextMeal ? { id: nextMeal.recipeId } : "skip",
);
              ...
              <div className="relative z-10">
                ...
                <div className="flex gap-6 mb-8 text-sahani-tertiary text-sm font-bold">
                  <span className="text-blue-400">
                    {nutritionTargets?.proteinGrams ?? 150}g protein
                  </span>
                  <span className="text-amber-400">
                    {nutritionTargets?.carbsGrams ?? 250}g carbs
                  </span>
                  <span className="text-rose-400">
                    {nutritionTargets?.fatGrams ?? 65}g fat
                  </span>
                </div>
                <p className="text-sahani-tertiary font-medium mb-8 max-w-sm">
                  Start by adding some recipes to your collection, then build
                ...
                <div className="flex gap-4">
                  ...
                  <Link to="/recipes">
                    <Button
                      variant="outline"
                      className="rounded-2xl h-14 px-10 font-black border-white/20 text-white hover:bg-white/10"
                    >
                      Browse Recipes
                    </Button>
                  </Link>
              ...
              <div className="flex-1">
                ...
                {nextRecipe ? (
                    ...
                    <div className="flex items-center gap-6 mb-8 text-sahani-tertiary">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-sm">
                          {(nextRecipe.prepTimeMinutes || 0) +
                            (nextRecipe.cookTimeMinutes || 0)}{" "}
                          mins
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                  ...
                  <>
                    ...
                    <p className="text-sahani-tertiary font-medium mb-8 max-w-sm">
                      You haven't assigned a recipe for your next meal. Let's
              ...
              {["breakfast", "lunch", "dinner", "snack"].map((type) => {
                ...
                return (
                  <SahaniCard
                    ...
                    className="group"
                  >
                    <p className="text-[10px] font-black text-sahani-tertiary uppercase tracking-widest mb-3">
                      {type}
                    </p>
                    {plan ? (
                      ...
                      <Link
                        to="/meal-planner"
                        className="flex flex-col items-center justify-center h-32 border-2 border-dashed border-border rounded-2xl hover:border-primary hover:bg-primary/5 transition-all"
                      >
                        <Plus className="w-5 h-5 text-sahani-tertiary group-hover:text-primary" />
                        <span className="text-[10px] font-black text-sahani-tertiary mt-1 group-hover:text-primary">
                          ADD
                        </span>
                      </Link>
                    )}
        ...
        <div className="col-span-4 space-y-8">
          ...
          <SahaniCard variant="tip">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link
                to="/shopping"
                className="w-8 h-8 rounded-xl bg-card flex items-center justify-center text-primary shadow-sm hover:scale-110 transition-transform"
              >
            ...
            <div className="space-y-3">
              {pendingShoppingItems.length > 0 ? (
                pendingShoppingItems.map((item, idx) => (
                  <div
                    key={idx}
                    className="bg-card p-4 rounded-2xl flex items-center gap-3 shadow-sm"
                  >
                ...
                <div className="text-center py-6">
                  <p className="text-xs font-bold text-sahani-tertiary">
                    Your list is empty.
                  </p>
                </div>
              )}
